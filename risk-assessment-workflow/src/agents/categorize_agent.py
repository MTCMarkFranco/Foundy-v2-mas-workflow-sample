...
async def evaluate_risk(agent: AgentRunnable, agent_name: str, client_id: str) -> RiskAssessment:
    """Run risk categorization for a client using a MAF FoundryAgent.

    Args:
        agent: A FoundryAgent (or any AgentRunnable) connected to CategorizeRiskAgent.
        agent_name: Display name for logging.
        client_id: The client identifier to evaluate.

    Returns:
        A validated RiskAssessment with locally-recomputed risk score.
    ...
    if assessment.rule_evaluations:
        ...
        if risk_level != assessment.risk_score:
            logger.warning(
                f"[AGENT:{agent_name}] Model returned risk_score="
    ...
    logger.info(
        f"[AGENT:{agent_name}] client={client_id} "
